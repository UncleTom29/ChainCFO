...
analyticsRouter.get("/apy-history", async (req: Request, res: Response) => {
  try {
    ...
    const timeSeries = reports.map((r: any) => {
      const totalBps = r.allocations.reduce((sum: number, a: any) => sum + a.basisPoints, 0);
      const weightedApy = totalBps > 0
        ? r.allocations.reduce((sum: number, a: any) => sum + (a.apy * a.basisPoints) / totalBps, 0)
        : 0;
      return {
...
analyticsRouter.get("/protocol-breakdown", async (_req: Request, res: Response) => {
  try {
    ...
    const totalBps = latestReport.allocations.reduce((sum: number, a: any) => sum + a.basisPoints, 0);
    const breakdown = latestReport.allocations.map((a: any) => ({
