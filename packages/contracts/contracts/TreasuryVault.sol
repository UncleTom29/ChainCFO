...
contract TreasuryVault is Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @notice Handle CRE EVMClient writeReport callback.
     * @param  report Encoded bytes reporting the result of the workflow.
     */
    function onReport(bytes calldata report) external nonReentrant onlyCreCaller {
        (
            ProtocolAllocation[] memory allocations,
            string memory llmRationale,
            uint256 totalValueUsd
        ) = abi.decode(report, (ProtocolAllocation[], string, uint256));

        _rebalance(allocations, llmRationale, totalValueUsd);
    }

    /**
     * @notice Internal logic for storing a new allocation report produced by the CRE workflow.
