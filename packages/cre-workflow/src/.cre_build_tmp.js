...
async function fetchAaveAPY(runtime2) {
  const client = new ClientCapability3;
  const fetchRaw = client.sendRequest(runtime2, async (req) => {
    ...
    const usdc = reserves.find((r) => r.symbol?.toUpperCase() === "USDC" || r.underlyingAsset?.toLowerCase() === USDC_MAINNET_ADDRESS);
    if (!usdc)
      throw new Error("fetchAaveAPY: USDC reserve not found");
    return {
  ...
  const result = await fetchRaw().result();
  return {
    name: "Aave v2",
    apy: result.apy,
...
async function fetchCompoundAPY(runtime2) {
  const client = new ClientCapability3;
  const fetchRaw = client.sendRequest(runtime2, async (req) => {
    const res = await req.sendRequest({
    ...
    if (!cToken)
      throw new Error("fetchCompoundAPY: cToken not found");
...
async function fetchMorphoAPY(runtime2) {
  const client = new ClientCapability3;
  const fetchRaw = client.sendRequest(runtime2, async (req) => {
    const query = `
      query {
    ...
    if (!market)
      throw new Error("fetchMorphoAPY: USDC market not found");
    return {
  ...
  return {
    ...
    tvlUsd: result.tvlUsd,
    chainId: 1,
    contractAddress: "morphoData"
  };
}
async function fetchAllProtocols(runtime2) {
  const results = await Promise.allSettled([
    fetchAaveAPY(runtime2),
    fetchCompoundAPY(runtime2),
    fetchMorphoAPY(runtime2)
  ]);
  const successful = [];
  for (const result of results) {
    if (result.status === "fulfilled") {
      ...
      runtime2.log(`fetchAllProtocols failed: ${result.reason.message}`);
    }
  }
  if (successful.length === 0) {
    runtime2.log("fetchAllProtocols: all live fetches failed, using deterministic fallback protocol set");
...
async function fetchCredoraRisk(runtime2, protocols) {
  runtime2.log(`Calculating structural risk scores locally for protocols: ${protocols.join(", ")}`);
  return protocols.map((p) => {
    ...
    if (name.includes("aave"))
      ...
      score = 82;
    else if (name.includes("morpho"))
      score = 75;
    else if (name.includes("maker"))
      score = 90;
...
async function fetchProofOfReserve(runtime2, feedAddress) {
  try {
    const client = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
    const abi = parseAbi([
      "function latestRoundData() view returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)"
    ]);
    const data = hexToBytes2(encodeFunctionData({
      abi,
      functionName: "latestRoundData"
    }));
    const reply = await client.callContract(runtime2, {
      call: {
        to: feedAddress,
    ...
    const now = Math.floor(runtime2.now().getTime() / 1000);
    ...
    runtime2.log(`fetchProofOfReserve: failed — ${err.message}`);
    return { verified: false, tvlReported: BigInt(0), lastUpdated: 0 };
  }
}
function equalDistribution(protocols, policy) {
  if (protocols.length === 0)
    return [];
  const available = 1e4 - policy.minLiquidityBufferBps;
...
async function rankAllocations(runtime2, protocols, riskScores, policy) {
...
Respond with valid JSON only, no markdown, no explanation outside the JSON:
{
  ...
  try {
    ...
    const apiKey = await runtime2.getSecret("OPENROUTER_API_KEY");
    if (!apiKey) {
      throw new Error("OPENROUTER_API_KEY secret is not configured");
    }
    const res = await client.sendRequest(runtime2, {
      request: {
    ...
    if (!text) {
      runtime2.log(`rankAllocations: unexpected LLM response structure: ${jsonText}`);
    }
    ...
    return {
      ...
      timestamp: runtime2.now().getTime()
    };
  } catch (err) {
    console.error("DEBUG:", err);
    runtime2.log(`rankAllocations: LLM failed, falling back to equal distribution — ${err.message}`);
    return {
...
function validateCompliance(allocations, policy) {
  const violations = [];
  const totalBps = allocations.reduce((sum, a) => sum + a.allocationBps, 0);
  const availableBps = 1e4 - policy.minLiquidityBufferBps;
  for (const a of allocations) {
    if (a.allocationBps > policy.maxAllocationBps) {
      violations.push(`${a.protocol} allocation ${a.allocationBps} bps exceeds max ${policy.maxAllocationBps} bps`);
    }
  }
  if (totalBps > availableBps) {
    violations.push(`Total allocation ${totalBps} bps exceeds available ${availableBps} bps (liquidity buffer: ${policy.minLiquidityBufferBps} bps)`);
  }
  if (allocations.length > policy.maxProtocols) {
    violations.push(`${allocations.length} protocols exceeds maximum ${policy.maxProtocols}`);
  }
  return { compliant: violations.length === 0, violations };
}
var PARITY_TOLERANCE = 0.005;
async function checkStablecoinPeg(runtime2, feedAddress) {
  try {
    const client = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
    const abi = parseAbi([
      "function latestRoundData() view returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)"
    ...
    const decoded = decodeFunctionResult({ abi, functionName: "latestRoundData", data: reply.data });
    const answer = decoded[1];
    const price = Number(answer) / 1e8;
    return price >= 1 - PARITY_TOLERANCE && price <= 1 + PARITY_TOLERANCE;
  } catch (err) {
    runtime2.log(`checkStablecoinPeg: failed — ${err.message}`);
    return false;
  }
}
async function executeRebalance(runtime2, allocations, vaultAddress, llmRationale, totalValueUsd) {
  const client = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
  ...
  runtime2.log(`Executing allocation for ${allocations.length} protocols`);
  for (const allocation of allocations) {
    runtime2.log(`Executing allocation: ${allocation.protocol} → ${allocation.allocationBps} bps`);
  }
  try {
    runtime2.log(`[executeRebalance] encoding ABI parameters...`);
    ...
    ], [allocationStructs, llmRationale, BigInt(Math.floor(totalValueUsd))]);
    runtime2.log(`[executeRebalance] creating report with runtime.report... type of runtime.report: ${typeof runtime2.report}`);
    const signedReportResponse = runtime2.report(prepareReportRequest(reportData)).result();
    runtime2.log(`[executeRebalance] calling client.writeReport... type of client.writeReport: ${typeof client.writeReport}`);
    const txResponse = client.writeReport(runtime2, {
      receiver: vaultAddress,
      report: signedReportResponse
    }).result();
    return txResponse?.txHash || "0x_mock_tx_if_no_hash_returned";
  } catch (err) {
    runtime2.log(`execute_rebalance error: ${err.message}
${err.stack}`);
    throw err;
  }
}
async function getGovernancePolicy(runtime2) {
  const client = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
  const abi = parseAbi([
  ...
  runtime2.log(`[getGovernancePolicy] Calling contract: ${runtime2.config.governanceContract}`);
  const callData = encodeFunctionData({ abi, functionName: "getPolicy" });
  runtime2.log(`[getGovernancePolicy] Calldata: ${callData}`);
  const resRaw = await client.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: runtime2.config.governanceContract,
      data: callData
    }),
    blockNumber: LATEST_BLOCK_NUMBER
  }).result();
  runtime2.log(`[getGovernancePolicy] Raw response: ${JSON.stringify(resRaw)}`);
  const decoded = decodeFunctionResult({ abi, functionName: "getPolicy", data: resRaw.data });
  return {
...
var runRebalanceWorkflow = async (runtime2, triggerType) => {
  const startTime = runtime2.now().getTime();
  runtime2.log(`[${triggerType}] Workflow start at ${startTime}`);
  let policy;
  try {
    policy = await getGovernancePolicy(runtime2);
    runtime2.log(`Policy retrieved: ${JSON.stringify(policy)}`);
  } catch (err) {
    runtime2.log(`policy_read error: ${err.message}. Using default fallback policy for this run.`);
    policy = {
  ...
  try {
    const feedAddress = runtime2.config.feedAddress || "0x95e10BaC2B89aB4D8508ccEC3f08494FcB3D23cb";
    const pegOk = await checkStablecoinPeg(runtime2, feedAddress);
    if (!pegOk) {
      runtime2.log("WARNING: USDC has depegged. Aborting.");
      return "Aborting: USDC has depegged";
    }
    runtime2.log("Peg check passed.");
  } catch (err) {
    runtime2.log(`peg_check error: ${err.message}`);
    throw err;
  }
  let protocols;
  try {
    protocols = await fetchAllProtocols(runtime2);
    runtime2.log(`Protocols fetched: ${protocols.length}`);
    if (protocols.length === 0) {
      throw new Error("No protocols available");
    }
  } catch (err) {
    runtime2.log(`fetch_protocols error: ${err.message}`);
    throw err;
  }
  let riskScores;
  try {
    riskScores = await fetchCredoraRisk(runtime2, protocols.map((p) => p.name));
    runtime2.log(`Risk scores fetched: ${riskScores.length}`);
  } catch (err) {
    runtime2.log(`fetch_risk error: ${err.message}`);
    throw err;
  }
  if (policy.requireProofOfReserve) {
    ...
    for (const p of protocols) {
      try {
        const porResult = await fetchProofOfReserve(runtime2, p.contractAddress);
        if (porResult.verified) {
          verifiedProtocols.push(p);
        } else {
          runtime2.log(`por_check: Protocol ${p.name} excluded`);
        }
      } catch (err) {
        runtime2.log(`por_check: Protocol ${p.name} error: ${err.message}`);
      }
    }
  ...
  try {
    allocationResult = await rankAllocations(runtime2, protocols, riskScores, {
    ...
    runtime2.log(`Allocations ranked: ${allocationResult.allocations.length}`);
  } catch (err) {
    runtime2.log(`rank_allocations error: ${err.message}`);
    throw err;
  }
  ...
  if (!complianceResult.compliant) {
    ...
    runtime2.log(`Compliance failed: ${reason}`);
    return reason;
  }
  runtime2.log("Compliance check passed.");
  const vaultAddress = runtime2.config.treasuryVaultAddress;
  let txHash;
  try {
    txHash = await executeRebalance(runtime2, allocationResult.allocations, vaultAddress, allocationResult.llmRationale, allocationResult.allocations.reduce((sum, a) => sum + a.allocationBps, 0));
    runtime2.log(`Rebalance executed: tx hash = ${txHash}`);
  } catch (err) {
    runtime2.log(`execute_rebalance error: ${err.message}`);
    throw err;
  }
  runtime2.log(`Workflow complete for ${triggerType}. Rationale: ${allocationResult.llmRationale}`);
  return "Success";
};
var onCronTrigger = async (runtime2) => {
  return await runRebalanceWorkflow(runtime2, "cron");
};
var onEvmLogTrigger = async (runtime2, log) => {
  runtime2.log(`EVM Event triggered from Block Hash: ${log.blockHash}`);
  return await runRebalanceWorkflow(runtime2, "evm_log");
};
